...
- 权限不足 → 检查飞书应用权限配置
- 用户权限问题 → 参考下方「用户权限问题排查」章节

**用户权限问题排查（重要）：**

当遇到「没有用户权限」或「权限不足」错误时，通常是用户个人授权未完成或身份配置异常导致的。按以下步骤排查：

1. **完成个人用户授权**
   - 在飞书对话中发送 `/feishu auth` 命令
   - 在弹出的 OAuth 授权页面勾选所有需要的权限（文档、日历、IM 等）
   - 完成个人账号授权
   - ⚠️ 注意：应用权限开通 ≠ 用户个人授权完成

2. **检查身份配置**
   - 编辑 `openclaw.json` 配置文件
   - 删除飞书渠道配置中的 `default_user_open_id` 参数（如果有配置的话）
   - 删除后插件会自动使用当前触发请求的用户身份调用 API
   - 避免固定使用单一用户身份导致的身份不匹配

3. **确认权限类型正确**
   - 到飞书开放平台的「权限管理」页面
   - 确认你申请的云文档、日历、IM 等相关权限是**用户级权限**（权限描述带「以用户身份调用」标识）
   - ❌ 不要申请应用级权限（带「以应用身份调用」标识）
   - 个人资源操作需要使用用户级权限

4. **刷新授权状态**
   - 完成以上配置后，重新发送 `/feishu auth` 完成授权
   - 再发送 `/feishu doctor` 检查用户授权状态是否恢复正常
